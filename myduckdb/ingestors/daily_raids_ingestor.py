...
DUCK_TABLE  = "raids_daily_events"
MYSQL_TABLE = "raids_daily_events"
...
class RaidsDuckIngestor:
    ...
    async def start(self):
        ...
        logger.success("🚀 DuckDB daily raids ingestor started.")
    ...
    async def stop(self):
        ...
        try:
            ...
            pass
        logger.info("🛑 DuckDB daily raids ingestor stopped.")
    ...
    async def _run_once(self, tag: str):
        try:
            ...
            logger.error(f"❌ DuckDB (raids) ingest [{tag}] failed: {e}", exc_info=True)
    ...
    def _ingest_window(self, tag: str):
        ...
        try:
            ...
            for day in targets:
                ...
                logger.success(f"🦆 Raids ingest {day} | synced={synced} mysql_rows={mysql_rows} duck_rows={duck_rows}")
        finally:
            try:
                con.close()
            except Exception:
    ...
    def _ensure_duck_schema(self, con: duckdb.DuckDBPyConnection):
        con.execute(f"""
            CREATE TABLE IF NOT EXISTS {DUCK_TABLE} (
              gym                 TEXT,
              raid_pokemon        SMALLINT,
              raid_level          SMALLINT,
              raid_form           VARCHAR,
              raid_team           SMALLINT,
              raid_costume        VARCHAR,
              raid_is_exclusive   TINYINT,
              raid_ex_raid_eligible TINYINT,
              area_id             SMALLINT,
              seen_at             TIMESTAMP,
              day_date            DATE
            )
        """)
        con.execute("""
            CREATE TABLE IF NOT EXISTS _meta_raids_daily_ingest (
    ...
    def _load_day_full(self, con: duckdb.DuckDBPyConnection, day: date) -> int:
        ...
        sel = (
            f"SELECT gym, raid_pokemon, raid_level, raid_form, raid_team, "
            f"       raid_costume, raid_is_exclusive, raid_ex_raid_eligible, "
            f"       area_id, CAST(seen_at AS TIMESTAMP) AS seen_at, day_date "
    ...
    def _read_meta(self, con, day):
        r = con.execute("""
            ...
            FROM _meta_raids_daily_ingest WHERE day_date = ?""", [day]).fetchone()
        if not r:
            return None
        return {
    ...
    def _write_meta(self, con, day, mysql_rows, duck_rows, stable_runs, finalized):
        con.execute("""
            INSERT INTO _meta_raids_daily_ingest
