...
DUCK_TABLE  = "invasions_daily_events"
MYSQL_TABLE = "invasions_daily_events"
...
class InvasionsDuckIngestor:
    ...
    async def start(self):
        ...
        logger.success("🚀 DuckDB invasions ingestor started.")
    ...
    async def stop(self):
        ...
        try:
            ...
            pass
        logger.info("🛑 DuckDB invasions ingestor stopped.")
    ...
    async def _run_once(self, tag: str):
        try:
            ...
            logger.error(f"❌ DuckDB (invasions) ingest [{tag}] failed: {e}", exc_info=True)
    ...
    def _ingest_window(self, tag: str):
        ...
        try:
            ...
            for day in targets:
                ...
                logger.success(f"🦆 Invasions ingest {day} | synced={synced} mysql_rows={mysql_rows} duck_rows={duck_rows}")
        finally:
            try:
                con.close()
            except Exception:
    ...
    def _ensure_duck_schema(self, con: duckdb.DuckDBPyConnection):
        ...
        """)
        con.execute("""
            CREATE TABLE IF NOT EXISTS _meta_invasions_daily_ingest (
    ...
    def _load_day_full(self, con: duckdb.DuckDBPyConnection, day: date) -> int:
        ...
        sel = (
            f"SELECT pokestop, display_type, `character` AS \"character\", grunt, confirmed, "
    ...
    def _read_meta(self, con, day):
        r = con.execute("""
            ...
            FROM _meta_invasions_daily_ingest WHERE day_date = ?""", [day]).fetchone()
        if not r:
            return None
        return {
    ...
    def _write_meta(self, con, day, mysql_rows, duck_rows, stable_runs, finalized):
        con.execute("""
            INSERT INTO _meta_invasions_daily_ingest
