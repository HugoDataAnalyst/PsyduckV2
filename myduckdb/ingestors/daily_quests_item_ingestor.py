...
DUCK_TABLE  = "quests_item_daily_events"
MYSQL_TABLE = "quests_item_daily_events"
...
class QuestsItemDuckIngestor:
    ...
    async def start(self):
        ...
        logger.success("🚀 DuckDB quests-item ingestor started.")
    ...
    async def stop(self):
        ...
        try:
            ...
            pass
        logger.info("🛑 DuckDB quests-item ingestor stopped.")
    ...
    async def _run_once(self, tag: str):
        try:
            ...
            logger.error(f"❌ DuckDB (quests-item) ingest [{tag}] failed: {e}", exc_info=True)
    ...
    def _ingest_window(self, tag: str):
        ...
        try:
            ...
            for day in targets:
                ...
                logger.success(f"🦆 Quests-Item ingest {day} | synced={synced} mysql_rows={mysql_rows} duck_rows={duck_rows}")
        finally:
            try:
                con.close()
            except Exception:
    ...
    def _ensure_duck_schema(self, con: duckdb.DuckDBPyConnection):
        con.execute(f"""
            CREATE TABLE IF NOT EXISTS {DUCK_TABLE} (
              ...
              task_type   SMALLINT,
              item_id     SMALLINT,
              item_amount SMALLINT
            )
        """)
        con.execute("""
            CREATE TABLE IF NOT EXISTS _meta_qitem_daily_ingest (
    ...
    def _load_day_full(self, con: duckdb.DuckDBPyConnection, day: date) -> int:
        ...
        sel = (
            ...
            f"       mode, task_type, item_id, item_amount "
    ...
    def _read_meta(self, con, day):
        r = con.execute("""
            ...
            FROM _meta_qitem_daily_ingest WHERE day_date = ?""", [day]).fetchone()
        if not r:
            return None
        return {
    ...
    def _write_meta(self, con, day, mysql_rows, duck_rows, stable_runs, finalized):
        con.execute("""
            INSERT INTO _meta_qitem_daily_ingest
